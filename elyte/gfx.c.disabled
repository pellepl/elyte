...
void gfx_rect(const gfx_ctx_t *ctx, const gfx_area_t *a, gfx_color_t c)
{
    gfx_hline(ctx, a->x0, a->x1, a->y0, c);
    gfx_hline(ctx, a->x0, a->x1, a->y1, c);
    gfx_vline(ctx, a->x0, a->y0, a->y1, c);
    gfx_vline(ctx, a->x1, a->y0, a->y1, c);
}
...
void gfx_line(const gfx_ctx_t *ctx, int x0, int y0, int x1, int y1, gfx_color_t c)
{
    if (x0 == x1)
    {
        gfx_vline(ctx, x0, y0, y1, c);
        return;
    }
    if (y0 == y1)
    {
        gfx_hline(ctx, x0, x1, y0, c);
        return;
    }
    int dx = x1 - x0;
    int dy = y1 - y0;

    // this assumes we're not drawing stupid lines totally or partially off clip,
    // if this is the case implement some cohen-sutherland clipping
    if (abs_i32(dx) < abs_i32(dy))
    {
        ...
        if (y0 >= ctx->clip.y1)
            return;
        const int sy_16 = (dy << 16) / abs_i32(dx);
        const int sx = dx > 0 ? 1 : -1;
        int y_16 = y0 << 16;
        int x = x0;
        while (y_16 < y1 << 16) {
            int ny_16 = y_16 + sy_16;
            gfx_vline(ctx, x, y_16 >> 16, min_i32(y1, (ny_16 >> 16) - 1), c);
            y_16 = ny_16;
            x += sx;
        }
    }
    else
    {
        ...
        const int sx_16 = (dx << 16) / abs_i32(dy);
        const int sy = dy > 0 ? 1 : -1;
        int x_16 = x0 << 16;
        int y = y0;
        while (x_16 < x1 << 16) {
            int nx_16 = x_16 + sx_16;
            gfx_hline(ctx, x_16 >> 16, min_i32(x1, (nx_16 >> 16) - 1), y, c);
            x_16 = nx_16;
            y += sy;
        }
    }
